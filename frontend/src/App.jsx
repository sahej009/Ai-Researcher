...
function App() {
  ...
  const fetchHistory = async () => {
    try {
      const res = await fetch(
        "https://ai-researcher-whvn.onrender.com/api/history",
      );
  ...
  const handleResearch = async () => {
    ...
    if (file) {
      ...
      try {
        const uploadRes = await fetch(
          "https://ai-researcher-whvn.onrender.com/api/upload",
          {
            method: "POST",
            body: formData,
          },
        ...
        if (uploadData.status === "success") {
          ...
          setLogs((prev) => [
            ...prev,
            `Upload complete! Initializing AI agents...`,
          ]);
        }
      } catch (err) {
    ...
    const streamUrl = `https://ai-researcher-whvn.onrender.com/api/research?topic=${encodeURIComponent(topic)}&file_path=${encodeURIComponent(uploadedFilePath)}`;
    ...
    <div className="dashboard">
      ...
      <div className="sidebar">
        ...
        <div className="history-list">
          ...
          {history.length === 0 && (
            <p className="no-history">No past research.</p>
          )}
        </div>
      </div>
